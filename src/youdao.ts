...
const getDicts = async (word: string): Promise<any> => {
   ...
   if (root.simple?.word?.length) for (const word of root.simple.word) {
      if (word.ukspeech) word.ukspeech = `${youdaoAudio}${word.ukspeech}`;
      if (word.usspeech) word.usspeech = `${youdaoAudio}${word.usspeech}`;
   }
   if (root.ec?.word?.length) for (const word of root.ec.word) {
      if (word.ukspeech) word.ukspeech = `${youdaoAudio}${word.ukspeech}`;
      if (word.usspeech) word.usspeech = `${youdaoAudio}${word.usspeech}`;
   }
   return root;
...
if (import.meta.main)
   for (const word of Deno.args) {
      console.log(await getDicts(word));
   }
